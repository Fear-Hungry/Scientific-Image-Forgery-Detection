...
def _build_criterion(name: str, cfg: dict):
    name = name.lower()
    if name == "bce_dice":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train baseline FPN+ConvNeXt")
    parser.add_argument("--config", default="configs/baseline_fpn_convnext.json", help="Config JSON")
    ...
    use_amp = bool(cfg.get("use_amp", True)) and device.startswith("cuda")
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        fold_dir = output_dir / "models" / f"fold_{fold_id}"
        fold_dir.mkdir(parents=True, exist_ok=True)
        split_path = output_dir / "splits" / f"fold_{fold_id}.json"
        ...
        for epoch in range(1, epochs + 1):
            train_stats = train_one_epoch(model, train_loader, criterion, optimizer, device, use_amp=use_amp)
            val_stats, val_dice = validate(model, val_loader, criterion, device)
            ...
            print(
                ...
                f"val_loss={val_stats.loss:.4f} val_dice={val_dice:.4f}"
            )
